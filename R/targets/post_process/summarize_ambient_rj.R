register_target(
  ...
  tar_target(
    ...
    name = summarize_ambient_rj,
    command = {
      
      
      # Container to hold output
      rl_summary_rj <- vector(mode = 'list', length = length(sample_ambient_rj))
      # loop over one animal/cee combination at a time
      for(i in 1:length(sample_ambient_rj)){
        
        my_cee_id <- sample_ambient_rj[[i]][["cee_id"]]
        my_deployid <- sample_ambient_rj[[i]][["deployid"]]
        my_cee_type <- cee_dat %>% 
        ...
        for(j in 1:nrow(animal)){
          ...
          rl_samples[[j]] <- data.frame(deployid = animal$deployid[j],
                                        ...
                                        track_id = animal$imputation_num[j],
                                        raw = rls)
        ...
        f = file.path(here::here('data/proc_data/rl_samples_rj'))
        dir.create(path = f, showWarnings = FALSE, recursive = TRUE)
        
        write_csv(rl_df, file = file.path(f, paste0(my_deployid, "_", my_cee_id, "_rl-samples-rj.csv")))
        ...
        ggsave(p, 
               filename = file.path(f, paste0(unique(animal$deployid), "_", my_cee_id, "_rl-summary-rj.png")),
               device = 'png',
               dpi = 'retina',
        ...
        # Store items in List 
        rl_summary_rj[[i]] <- list(deployid = unique(animal$deployid),
                            cee_id = my_cee_id,
                            rl_raw = animal,
                            rl_summary = rl_df,
                            rl_quantiles = rl_sum,
