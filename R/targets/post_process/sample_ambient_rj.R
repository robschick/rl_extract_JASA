      ...
      for(i in 1:length(calc_rl_rj)){
        ...
        saveRDS(animal, file = here::here('data/proc_data', 
                                          paste0(unique(animal$deployid), "_", 
                                                 my_cee_id, '_tobl_rj.rds')))
        
        # Store items in List 
        rl_tobls_rj[[i]] <- list(deployid = unique(animal$deployid),
                            cee_id = my_cee_id,
                            pts_tobl = animal)
